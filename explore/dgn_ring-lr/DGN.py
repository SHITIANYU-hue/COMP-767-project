...
class DGN:
    ...
    def buildCriticNetwork(self, d=128,dv=16,dout=128,nv=8):
        ...
        with tf.variable_scope('update_Critic_network' + self.name):
            # enc
            f_dim = 128
            encode_layer1 = tf.keras.layers.Dense(128, activation=tf.nn.relu,
                                            kernel_initializer=init_w, bias_initializer=init_b, name='encoder_l1',
                                            trainable=True) ##change the encoding dim
            encode_layer2 = tf.keras.layers.Dense(f_dim, activation=tf.nn.relu,
        ...
        self.v_trainOp = tf.train.AdamOptimizer(learning_rate=0.001).minimize(self.v_loss)   ###to change
    ...
    def buildActorNetwork(self,d=128,dv=16,dout=128,nv=8):
        ...
        for i in range(self.agent_num):
            ...
            self.p_trainOp.append(
                tf.train.AdamOptimizer(learning_rate=0.00025).apply_gradients(grads_and_vars, name="apply_gradients"))
