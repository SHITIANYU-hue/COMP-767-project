...
sess = tf.Session()
agent = DGN(state_dim = 3, action_dim = 1, neighbors=neighbors,agent_num=agent_num,sess=sess)
...
if train_test==1: ##train the model
    for i in range(num_runs):
        ...
        for j in range(num_steps):
            ...
            action_dict = {}
            a=sess.run(agent.action, feed_dict={agent.state_holder: state_})
            k=0
            ...
            ret += np.average(rewards) ##here we consider the mean rewards of each agent
            ret_list.append(rewards)
     
         
            agent.remember( state_, a, rewards, next_state_,0. )
