...
class DGN:
    ...
    def buildCriticNetwork(self, d=128,dv=16,dout=128,nv=8):
        ...
        with tf.variable_scope('update_Critic_network' + self.name):
            ...
            encode_layer3 = tf.keras.layers.LSTM(f_dim, activation=tf.nn.relu,
                                            kernel_initializer=init_w, bias_initializer=init_b, name='encoder_l3',
                                            trainable=True)                               

            for i in range(self.agent_num):
                e1 = encode_layer1(self.state_holder[:, i * self.state_dim:(i + 1) * self.state_dim])
                # e1=tf.expand_dims(e1,-1)
                feature = encode_layer3(e1)
                feature = encode_layer2(feature)
        ...
        with tf.variable_scope('update_Actor_network' + self.name):
            ...
            for i in range(self.agent_num):
                ...
                encode_layer1 = tf.layers.Dense(f_dim, activation=tf.nn.relu,
                                            ...
                                            trainable=True)
                encode_layer2 = tf.keras.layers.LSTM(f_dim, activation=tf.nn.relu,
                                            kernel_initializer=init_w, bias_initializer=init_b, name='encoder_l2'+str(i),
                                            trainable=True)                
                ...
                self.action_sigma = tf.layers.Dense(1, activation=None,
                                                  ...
                                                  trainable=True)  
                h = encode_layer2(self.state_holder[:, i * self.state_dim:(i + 1) * self.state_dim])
                h = encode_layer1(h)
        ...
        with tf.variable_scope('target_Actor_network' + self.name):
            ...
            for i in range(self.agent_num):
                ...
                self.action_sigma_old = tf.layers.Dense(1, activation=None,
                                              ...
                                              trainable=True)
                h_old = encode_layer2(self.state_holder[:, i * self.state_dim:(i + 1) * self.state_dim])
                h_old = encode_layer1(h_old)
